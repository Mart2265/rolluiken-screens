...
import { ShieldCheck, SunMedium, Snowflake, Ruler, Wrench, Clock, Euro, Star, CheckCircle2, PhoneCall, ChevronRight, Palette, Sparkles, MessageCircle } from 'lucide-react';
...
export default function Home(){
  ...
  const WA = process.env.NEXT_PUBLIC_WHATSAPP_NR || '31623717058';
  ...
  async function submitQuote(e: React.FormEvent) {
    e.preventDefault();
    const res = await fetch('/api/quote', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ lead, price, config:{ type, widthCm, heightCm, bediening, lamellen, omkasting, montage } }) });
    if(res.ok) alert('Bedankt! We mailen/appen je binnen 1 werkdag.');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 text-slate-900">
      ...
      <header className="sticky top-0 z-50 backdrop-blur bg-white/70 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold tracking-tight">Rolluikenvoorjou</span>
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center relative">
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.5}}>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <a href="#afspraak" className="btn-secondary">Offerte</a>
              </div>
      ...
      <Section id="kleuren">
        ...
        <p className="muted mt-2 max-w-3xl">Populaire gevelkleuren en compacte kasten. Solar‑screens mogelijk zonder 230V. (Exacte RAL-kleur kiezen we tijdens inmeten.)</p>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="card bg-white shadow-sm self-end">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                ...
                <a href="#afspraak" className="btn-primary text-center">Offerte</a>
              </div>
      ...
      <div className="bg-white border-y border-slate-200">
        <Section id="reviews">
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-8">
            {[
              { name:'Daan, Berkel en Rodenrijs', text:'Top advies en het bleef heerlijk koel in de hitte.' },
              { name:'Fatima, Rotterdam Kralingen', text:'Slanke screens, goede prijs en snelle montage.' },
              { name:'Jeroen, Capelle a/d IJssel', text:'Binnen drie weken geplaatst. Helemaal verduisterd.' },
            ].map((r, i) => (
              <div key={i} className="subcard">
                <div className="flex items-center gap-1">{Array.from({ length: 5 }).map((_, k) => (<Star key={k} className="w-4 h-4 fill-amber-400 text-amber-400" />))}</div>
      ...
      {/* Sticky WhatsApp + Offerte bar */}
      <div className="stickybar">
        <a href={waHref} target="_blank" className="btn-secondary inline-flex items-center"><MessageCircle className="w-4 h-4 mr-1"/>WhatsApp</a>
        <a href="#afspraak" className="btn-primary inline-flex items-center">Offerte <ChevronRight className="w-4 h-4 ml-1"/></a>
      </div>

      <footer className="border-t border-slate-200 bg-white mt-20">
        <Section>
          <div className="text-sm text-slate-600 grid md:grid-cols-2 gap-4">
            <div>© {new Date().getFullYear()} Rolluikenvoorjou. Alle rechten voorbehouden.</div>
            <div className="md:text-right">Contact: mart_2265@hotmail.com · WhatsApp: +31 6 23717058</div>
          </div>
        </Section>
      </footer>
    </div>
